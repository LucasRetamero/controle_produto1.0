...
{
	protected $estoqueDAO,$enderecoDAO, $subEspecieDAO, $produtoDAO, $tipoEnderecoDAO;
	
	public function __construct(EstoqueDAO $estoque_dao, SubEspecieDAO $subEspecie_dao, EnderecoDAO $endereco_dao, ProdutosDAO $produtos_dao, TipoEnderecoDAO $tipoEndereco_dao){
	...
	return view('dashboard.cadastro.estoque.estoqueForm',['dadosSubEspecie' => $this->subEspecieDAO->getAllSubEspecie(), 'dadosEndereco' => $this->enderecoDAO->getAllDAO(), 'dadosTipoEndereco' => $this->tipoEnderecoDAO->getAllDAO() ]);
	}
	...
	public function getEstoqueFormWithProduct(Request $request){
	  if($this->produtoDAO->getLikeCodigoDAO($request->input('nomeProdutoQuery'))->count() > 0){
		return view('dashboard.cadastro.estoque.estoqueForm',['dadosSubEspecie' => $this->subEspecieDAO->getAllSubEspecie(), 'dadosEndereco' => $this->enderecoDAO->getAllDAO(), 'dadosTipoEndereco' => $this->tipoEnderecoDAO->getAllDAO() ,'msgSuccess' => "Produto Encontrado !", 'dadosProduto' => $this->produtoDAO->getLikeCodigoDAO($request->input('nomeProdutoQuery')) ] );			
	  }else{
		return view('dashboard.cadastro.estoque.estoqueForm',['dadosSubEspecie' => $this->subEspecieDAO->getAllSubEspecie(), 'dadosEndereco' => $this->enderecoDAO->getAllDAO(), 'dadosTipoEndereco' => $this->tipoEnderecoDAO->getAllDAO() ,'msgError' => "Nenhum produto encontrado !"] );		 
	  }
	}
	...
	public function actionsListeTable($id, $option){
	  switch($option){
		case "edit":
         return view('dashboard.cadastro.estoque.estoqueForm',[ 'dadosLogistico'=>$this->estoqueDAO->getIdEstoqueDAO($id), 'dadosSubEspecie' => $this->subEspecieDAO->getAllSubEspecie(), 'dadosEndereco' => $this->enderecoDAO->getAllDAO(), 'dadosTipoEndereco' => $this->tipoEnderecoDAO->getAllDAO()]);	
        break;
	...
	return view('dashboard.cadastro.estoque.estoqueForm',['dadosSubEspecie' => $this->subEspecieDAO->getAllSubEspecie(), 'dadosEndereco' => $this->enderecoDAO->getAllDAO(), 'dadosTipoEndereco' => $this->tipoEnderecoDAO->getAllDAO() ,'msgSuccess' => $msg]);		
	}
