...
@section('content')
    ...
    <div class="jumbotron bg-primary">
        ...
        <form method="post" action="{{ route('dashboard.cadastro.lote.loteForm.actionsMenu') }}" class="form-signin"
            ...
            @if (!empty(Auth::User()->empresa_id))
                @if (isset($dadosLote))
                    ...
                    <input type="hidden" id="empresa_id" name="empresa_id" value="{{ $dadosLote[0]->empresa_id }}">
                @endif
            @endif
            ...
            <div class="form-group">
                ...
                @if (isset($dadosLote))
                    <input type="text" class="form-control" id="txtLote" name="lote"
                        value="{{ $dadosLote[0]->lote }}" required>
                @else
                    <input type="text" class="form-control" id="txtLote" name="lote"
                        placeholder="Digite o Lote..." required>
                @endif
                <small id="txtEmail" class="form-text text-muted">
                    <!-- Small message -->
                </small>
            </div>
            ...
            @endif
            </br><a href="{{ route('dashboard.cadastro.lote') }}" class="btn btn-light btn-block"
                style="font-size:x-large;"><img src="{{ asset('img\icons\NoIcon.png') }}" width="40px"
                    height="40px"></img>Cancelar</a>
