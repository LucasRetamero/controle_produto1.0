...
@section('content')
    ...
    <div class="container" id="topPage">
        <h1 class="h2">Cadastro / <a href="{{ route('dashboard.cadastro.endereco') }}">Endereço</a> / Formulário: Endereço
        </h1>
    ...
    <div class="jumbotron bg-primary">
        ...
        <form method="post" action="{{ route('dashboard.cadastro.endereco.enderecoForm.actionsMenu') }}"
            class="form-signin" onsubmit="return confirm('Deseja realmente executar essa açâo ?');">

            <input type="hidden" name="_token" id="csrf-token" value="{{ Session::token() }}" />

            <!-- Codigo -->
            @if (isset($dadosEndereco) && !empty(Auth::User()->empresa_id))
                ...
                <div class="col-sm-9">
                    @if (isset($dadosEndereco))
                        <input type="text" class="form-control" id="txtEndereco" name="endereco"
                            value="{{ $dadosEndereco[0]->endereco }}" placeholder="Digite o endereço..." required autofocus>
                    @else
                        <input type="text" class="form-control" id="txtEndereco" name="endereco"
            ...
            @endif
            </br><a href="{{ route('dashboard.cadastro.endereco') }}" class="btn btn-light btn-block"
                style="font-size:x-large;"><img src="{{ asset('img\icons\leftArrowIcon.png') }}" width="40px"
                    height="40px"></img>Lista de Endereço</a>
